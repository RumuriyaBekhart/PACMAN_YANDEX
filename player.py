'''player.py - логика поведения игрока, анимация'''
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, filename):
        ...
        image = pygame.image.load("images/pacman2.png").convert()
        # создание анимации pacman
        self.move_right_animation = Animation(image, 32, 32)
        self.move_left_animation = Animation(pygame.transform.flip(image, True, False), 32, 32)
        self.move_up_animation = Animation(pygame.transform.rotate(image, 90), 32, 32)
        self.move_down_animation = Animation(pygame.transform.rotate(image, 270), 32, 32)
        image = pygame.image.load("images/pacman3.png").convert()
        self.explosion_animation = Animation(image, 30, 30)
        self.player_image = pygame.image.load(filename).convert()
        self.player_image.set_colorkey(BLACK)

    def update(self, horizontal_block, vertical_block):
        if not self.eating:
            ...
            # вверх/вниз
            for block in pygame.sprite.spritecollide(self, horizontal_block, False):
                self.rect.centery = block.rect.centery
                self.change_y = 0
            for block in pygame.sprite.spritecollide(self, vertical_block, False):
                self.rect.centerx = block.rect.centerx
                self.change_x = 0

            # запуск анимации
            if self.change_x > 0:
                self.move_right_animation.update(11)
                self.image = self.move_right_animation.get_current_image()
            elif self.change_x < 0:
                self.move_left_animation.update(11)
                self.image = self.move_left_animation.get_current_image()

            if self.change_y > 0:
                self.move_down_animation.update(11)
                self.image = self.move_down_animation.get_current_image()
            elif self.change_y < 0:
                self.move_up_animation.update(11)
...
class Animation(object):
    def __init__(self, img, width, height):
        ...
        # переменная, содержащая текущее изображение списка
        self.index = 0
        # переменная для хранения времяни
        self.clock = 1
